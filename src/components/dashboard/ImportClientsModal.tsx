...
const COLUMN_OPTIONS = ['email', 'prenom', 'nom', 'telephone', 'ville', 'ca_total', '(ignorer)']

const SAMPLE_CSV =
  'Email,Prénom,Nom,Téléphone,Ville\n' +
  'sophie.martin@gmail.com,Sophie,Martin,06 12 34 56 78,Paris\n' +
  'thomas.bernard@orange.fr,Thomas,Bernard,07 23 45 67 89,Lyon\n' +
  'camille.dubois@gmail.com,Camille,Dubois,06 34 56 78 90,Bordeaux\n'
...
export default function ImportClientsModal({ isOpen, onClose, onSuccess }: ImportClientsModalProps) {
  ...
  const normalizeHeader = (h: string): string => {
    ...
    if (['email'].includes(n)) return 'email'
    if (['prenom', 'firstname', 'first_name', 'prenom'].includes(n)) return 'prenom'
    if (['nom', 'lastname', 'last_name'].includes(n)) return 'nom'
    if (['telephone', 'phone', 'tel'].includes(n)) return 'telephone'
    if (['ville', 'city'].includes(n)) return 'ville'
    if (['ca_total', 'ca total', 'catotal', 'chiffre affaires'].includes(n)) return 'ca_total'
    return '(ignorer)'
  }
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      // If column mapping differs from default headers, rebuild the file
      let uploadFile = file

      // Check if we need to remap (headers differ from mapped keys)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
          <div>
            <h2 className="text-lg font-bold text-white">Importer des clients</h2>
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 p-4 bg-emerald-900/30 border border-emerald-700/40 rounded-xl">
                  ...
                  <div>
                    <p className="text-white font-semibold">
                      {result.imported} client{result.imported > 1 ? 's' : ''} importé{result.imported > 1 ? 's' : ''} avec succès
                    </p>
                    {result.skipped > 0 && (
              ...
              {result.imported === 0 && (
                <div className="flex items-center gap-3 p-4 bg-red-900/30 border border-red-700/40 rounded-xl">
                  ...
                  <p className="text-white font-semibold">Aucun contact importé</p>
                </div>
              )}
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    Importer {previewRows.length > 0 ? 'les contacts' : ''}
                  </>
                )}
              </button>
            </>
